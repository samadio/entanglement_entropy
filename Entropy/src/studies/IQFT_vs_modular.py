...
for i in range(len(numbers)):
    ...
    for k in range(1, 2 * L + 1):
        ...
        results = [qinfo.entropy(qinfo.partial_trace(current_state, chosen)) for chosen in sampled_bipartitions[i][k - 1]]
...
for i in range(len(numbers)):
    ...
    for k in range(1, 2 * L + 1):
        ...
        results = [entropy(k, L, chosen, nonzero_binary) for chosen in sampled_bipartitions[i][k - 1]]
