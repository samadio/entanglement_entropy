...
current_state = 0
number_of_bip = 100
means = [np.ones((len(numbers), 2 * L), dtype=float) * 512 for L in L_list]
stds = [np.ones((len(numbers), 2 * L), dtype=float) * 512 for L in L_list]
start = time()

for i, N in enumerate(numbers):
    L = L_list[i]
    for k in range(1, 2 * L + 1):
        current_state = construct_modular_state(k, L, aux.nonzeros_decimal(k, Y, N))
        number_qubits = k + L
        bipartitions = [bip.random_bipartition(number_qubits, number_qubits // 2) for i in range(number_of_bip)]
        times = []
